	...
	tests := map[string]struct {
		...
		"success on valid response": {
			...
			wantCollected: map[string]int64{
				...
				"org_apache_cassandra_metrics_table_count_CompactionBytesWritten": 679,
				"org_apache_cassandra_metrics_table_count_LiveDiskSpaceUsed":      257710,
				"org_apache_cassandra_metrics_table_count_PendingCompactions":     0,
				"org_apache_cassandra_metrics_table_count_TotalDiskSpaceUsed":     257710,
				"system_up_time": 0,
			},
		},
		"fails if endpoint returns invalid data": {
