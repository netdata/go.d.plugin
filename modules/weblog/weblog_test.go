...
func TestWebLog_guessParser(t *testing.T) {
	...
	tests := []test{
		{
			name:           "csv vhost custom 4",
			input:          `test.example.com:80 88.191.254.20 - - [22/Mar/2009:09:30:31 +0100] "GET / HTTP/1.0" 200 8674 "-" "-" 8674 0.123 0.123,0.321`,
			wantCSVFormat:  csvVhostCustom4,
			wantParserType: logs.TypeCSV,
		},
		{
			name:           "csv vhost custom 3",
			input:          `test.example.com:80 88.191.254.20 - - [22/Mar/2009:09:30:31 +0100] "GET / HTTP/1.0" 200 8674 "-" "-" 8674 0.123`,
			wantCSVFormat:  csvVhostCustom3,
			wantParserType: logs.TypeCSV,
		},
		{
			name:           "csv vhost custom 2",
			input:          `test.example.com:80 88.191.254.20 - - [22/Mar/2009:09:30:31 +0100] "GET / HTTP/1.0" 200 8674 8674 0.123 0.123,0.321`,
			wantCSVFormat:  csvVhostCustom2,
			wantParserType: logs.TypeCSV,
		},
		{
			name:           "csv vhost custom 1",
			input:          `test.example.com:80 88.191.254.20 - - [22/Mar/2009:09:30:31 +0100] "GET / HTTP/1.0" 200 8674 8674 0.123`,
			wantCSVFormat:  csvVhostCustom1,
			wantParserType: logs.TypeCSV,
		},
		{
			name:           "csv vhost host common",
			input:          `test.example.com:80 88.191.254.20 - - [22/Mar/2009:09:30:31 +0100] "GET / HTTP/1.0" 200 8674`,
			wantCSVFormat:  csvVhostCommon,
			wantParserType: logs.TypeCSV,
		},
		{
			name:           "csv custom 4",
			input:          `88.191.254.20 - - [22/Mar/2009:09:30:31 +0100] "GET / HTTP/1.0" 200 8674 "-" "-" 8674 0.123 0.123,0.321`,
			wantCSVFormat:  csvCustom4,
			wantParserType: logs.TypeCSV,
		},
		{
			name:           "csv custom 3",
			input:          `88.191.254.20 - - [22/Mar/2009:09:30:31 +0100] "GET / HTTP/1.0" 200 8674 "-" "-" 8674 0.123`,
		...
		{
			name:           "csv host common",
			input:          `88.191.254.20 - - [22/Mar/2009:09:30:31 +0100] "GET / HTTP/1.0" 200 8674`,
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			p, err := weblog.newParser([]byte(tc.input))
			if tc.wantErr {
				assert.Error(t, err)
				return
			}
