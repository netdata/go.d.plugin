...
//				return fmt.Errorf("'%s' fieldID bad syntax: '%s'", k, v)
//			}
//		case keyCode:
...
//			if !ok {
//				return fmt.Errorf("unparsable '%s' fieldID : '%s'", k, v)
//			}
//			if !reHTTPMethod.MatchString(gm.get(keyMethod)) {
//				return fmt.Errorf("'%s' fieldID bad syntax : '%s'", keyMethod, gm.get(keyMethod))
//			}
