				...
				return &runSim{
					do: func(mgr *Manager) {
						mgr.processDiscoveredConfigGroups([]*confgroup.Group{
							prepareCfgGroup(stockCfg.Source(), stockCfg),
							prepareCfgGroup(userCfg.Source(), userCfg),
							prepareCfgGroup(discCfg.Source(), discCfg),
						})
						mgr.dyncfgConfig(functions.Function{
							UID:  "1-remove",
					...
					wantRunning: nil,
					wantDyncfg: `
CONFIG go.d:collector:success:stock create running job /collectors/jobs stock 'type=stock,module=success,job=stock' 'schema get restart' 0x0000 0x0000

CONFIG go.d:collector:success:user create running job /collectors/jobs user 'type=user,module=success,job=user' 'schema get restart' 0x0000 0x0000

CONFIG go.d:collector:success:discovered create running job /collectors/jobs discovered 'type=discovered,module=success,job=discovered' 'schema get restart' 0x0000 0x0000
...
func prepareCfgGroup(src string, configs ...confgroup.Config) *confgroup.Group {
